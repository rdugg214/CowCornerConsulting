...
if i>=2 && i<= Nz-1 && j >=2 && j<= Nx-1
    flux_n = - k_face(k_new,dz,index,Nx) * Kzz(index) * (((h(index+Nx) -(sigmaz/2)*(h(index+Nx) - h(index)))/dz(index)) + 1);
    flux_w = - k_face(k_new,dx,index,-1) * Kxx(index) * ((h(index-1) +(sigmax/2)*(h(index) - h(index-1)))/dx(index-1));
    flux_s = - k_face(k_new,dz,index,-Nx)* Kzz(index) * (((h(index) -(sigmaz/2)*(h(index) - h(index-Nx)))/dz(index-Nx)) + 1);
    flux_e = - k_face(k_new,dx,index,1)* Kxx(index) *((h(index) +(sigmax/2)*(h(index+1) - h(index)))/dx(index));
    
    flux_n_old = - k_face(k_old,dz,index,Nx) * Kzz(index) * (((h_old(index+Nx)-(sigmaz/2)*(h_old(index+Nx)- h_old(index)))/dz(index)) + 1);
    flux_w_old = - k_face(k_old,dx,index,-1) * Kxx(index) * ((h_old(index-1)+(sigmax/2)*(h_old(index)- h_old(index-1)))/dx(index-1));
    flux_s_old = - k_face(k_old,dz,index,-Nx) * Kzz(index) *  ((h_old(index)-(sigmaz/2)*(h_old(index)- h_old(index-Nx)))/dz(index-Nx) + 1);
    flux_e_old = - k_face(k_old,dx,index,1) * Kxx(index) * ((h_old(index)+(sigmax/2)*(h_old(index+1)- h_old(index)))/dx(index));
elseif i == 1 && j == 1
    flux_n = - k_face(k_new,dz,index,Nx) * Kzz(index) * (((h(index+Nx) -(sigmaz/2)*(h(index+Nx) - h(index)))/dz(index)) + 1);
    flux_e = - k_face(k_new,dx,index,1)* Kxx(index) *((h(index) +(sigmax/2)*(h(index+1) - h(index)))/dx(index));
    flux_n_old = - k_face(k_old,dz,index,Nx) * Kzz(index) * (((h_old(index+Nx)-(sigmaz/2)*(h_old(index+Nx)- h_old(index)))/dz(index)) + 1);
